...
class Peep
  ...
  def self.create(peep:, userid:)
    ...
    result = DatabaseConnection.query(
      "INSERT INTO chitters (peep, time, userid)
      VALUES('#{peep}', '#{time}', '#{userid}')
      RETURNING id, peep, time, userid;"
    )
  ...
  def self.update(id:, peep:)
    time = Time.new.strftime('%d/%m/%Y %k:%M')
    result = DatabaseConnection.query(
      "UPDATE chitters SET peep = '#{peep}', time = '#{time}'
       WHERE id = #{id} RETURNING id, peep, time, userid;"
    )
    Peep.new(id: result[0]['id'], peep: result[0]['peep'], time: result[0]['time'], userid: peep['userid'])
  end
